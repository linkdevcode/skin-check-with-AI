...
export default async function RoutineNganSachDetailPage({ params }: Props) {
  ...
  if (!session?.user?.id) {
    redirect(`/dang-nhap?callbackUrl=/routine-ngan-sach/${(await params).id}`);
  }
  const { id } = await params;
  ...
  return (
    <div className="min-h-dvh bg-slate-50 px-4 pb-24 pt-6 dark:bg-[#0b0e14]">
      <div className="mx-auto max-w-lg sm:max-w-xl">
        ...
        <div className="relative mt-6 aspect-[3/4] w-full max-w-[220px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-zinc-800 dark:bg-zinc-900">
          <Image src={row.imageUrl} alt="Ảnh đã phân tích" fill className="object-cover" sizes="220px" />
        </div>
